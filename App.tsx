...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ohca-app-state-v6') || localStorage.getItem('ohca-app-state-v5') || localStorage.getItem('ohca-app-state-v4') || localStorage.getItem('ohca-app-state-v3');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('ohca-app-state-v6', JSON.stringify(data));
  }, [data]);
      ...
      <header className="fixed top-0 left-0 right-0 bg-medical-600/95 backdrop-blur-md shadow-lg shadow-medical-900/10 z-40 px-4 py-2 flex justify-between items-center border-b border-accent-500/70">
        <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
                ...
                <span className="text-[10px] text-accent-200 font-mono">Ver.6</span>
            </div>
        </div>
